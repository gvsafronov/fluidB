...
fluidB is often used in the context of demanding use cases, where it
...
While fluidB is an in memory system, it deals with the operating system in
different ways, for example, in the context of persisting to disk.
Moreover fluidB implements a rich set of commands. Certain commands
are fast and run in constant or logarithmic time, other commands are slower
O(N) commands, that can cause latency spikes.

For all these reasons, fluidB 2.8.13 introduced a new feature called
...
details, however for more information about the general topic of fluidB
and latency, please read the [fluidB latency problems troubleshooting](/topics/latency) page in this documentation.
...
a very specific operation performed by fluidB. For example the `fork` event
only monitors the time taken by fluidB to execute the `fork(2)` system call.
...
By default monitoring is disabled (threshold set to 0), even if the actual cost of latency monitoring is near zero. However while the memory requirements of latency monitoring are very small, there is no good reason to raise the baseline memory usage of a fluidB instance that is working well.
...
Like many other fluidB commands, `LATENCY` accept subcommands that modify the
...
All time does not really mean the maximum latency since the fluidB instance was
...
This is the output you should post in the fluidB mailing list if you are
...
Example output:
    ...
    Dave, I have observed latency spikes in this fluidB instance.
    ...
    - Check your Slow Log to understand what are the commands you are
      ...
      http://fluidB.io/commands/slowlog for more information.
