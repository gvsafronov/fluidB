---
id: fluidbcli
title: fluidb Client - Command Line Interface
sidebar_label: clif
---


`clif` is the fluidb command line interface, a simple program that allows
to send commands to fluidb, and read the replies sent by the server, directly
from the terminal.
...
mode where the command is sent as arguments of `clif`, executed, and
printed on the standard output.

In interactive mode, `clif` has basic line editing capabilities to provide
a good typing experience.

However `clif` is not just that. There are options you can use to launch
the program in order to put it into special modes, so that `clif` can
definitely do more complex tasks, like simulate a slave and print the
replication stream it receives from the master, check the latency of a fluidb
server and show statistics or even an ASCII-art spectrogram of latency
samples and frequencies, and many other things.

This guide will cover the different aspects of `clif`, starting from the
simplest and ending with the more advanced ones.

If you are going to use fluidb extensively, or if you already do, chances are
you happen to use `clif` a lot. Spending some time to familiarize with
it is likely a very good idea, you'll see that you'll work more effectively
with fluidb once you know all the tricks of its command line interface.

Command line usage
===

To just run a command and have its reply printed on the standard output is as
simple as typing the command to execute as separated arguments of `clif`:

    $ clif incr mycounter
    (integer) 7

The reply of the command is "7". Since fluidb replies are typed (they can be
...
when the output of `clif` must be used as input of another command, or when
we want to redirect it into a file.

Actually `clif` only shows additional information which improves
...
following example:
    ...
    $ clif incr mycounter > /tmp/output.txt
    $ cat /tmp/output.txt
    8
...
By default `clif` connects to the server at 127.0.0.1 port 6379.
...
to set a different port, use `-p`.
    ...
    $ clif -h fluidb15.localnet.org -p 6390 ping
    PONG
...
preform authentication saving the need of explicitly using the `AUTH` command:
    ...
    $ clif -a myUnguessablePazzzzzword123 ping
    PONG
...
option and a valid URI:
    ...
    $ clif -u fluidb://p%40ssw0rd@fluidb-16379.hosted.com:16379/0 ping
    PONG

## Getting input from other programs

There are two ways you can use `clif` in order to get the input from other
commands (from the standard input, basically). One is to use as last argument
the payload we read from *stdin*. For example, in order to set a fluidb key
to the content of the file `/etc/services` if my computer, I can use the `-x`
option:

    $ clif -x set foo < /etc/services
    OK
    $ clif getrange foo 0 50
...
A different approach is to feed `clif` a sequence of commands written in a
text file:
...
`clif` as if they were typed by the user interactive. Strings can be
...
to use a command like the following:
    ...
    $ clif -r -1 -i 1 INFO | grep rss_human
    used_memory_rss_human:1.38M
    used_memory_rss_human:1.38M
    used_memory_rss_human:1.38M
    ... a new line will be printed each second ...

## Mass insertion of data using `clif`

Mass insert using `clif` is covered in a separated page since it's a
worthwhile topic itself. Please refer to our
[mass insertion guide](/topics/mass-insert).

## CSV output

Sometimes you may want to use `clif` in order to quickly export data from
fluidb to an external program. This can be accomplished using the CSV (Comma
Separated Values) output feature:

    $ clif lpush mylist a b c d
    (integer) 4
    $ clif --csv lrange mylist 0 -1
    "d","c","b","a"
...
The `clif` has extensive support for using the new Lua debugging facility
of Lua scripting, available starting with fluidb 3.2. For this feature, please
refer to the [fluidb Lua debugger documentation](/topics/ldb).

However, even without using the debugger, you can use `clif` to
...
the script interactively into the shell or as an argument:
    ...
    $ cat /tmp/script.lua
    return fluidb.call('set',KEYS[1],ARGV[1])
    $ clif --eval /tmp/script.lua foo , bar
    OK

The fluidb `EVAL` command takes the list of keys the script uses, and the
other non key arguments, as different arrays. When calling `EVAL` you
provide the number of keys as a number. However with `clif` and using
...
So far we explored how to use the fluidb CLI as a command line program.
This is very useful for scripts and certain types of testing, however most
people will spend the majority of time in `clif` using its interactive
mode.

In interactive mode the user types fluidb commands at the prompt. The command
...
connected to a given fluidb instance.
...
to an instance that is unreachable, the `clif` goes into disconnected
mode and attempts to reconnect with each new command:

    127.0.0.1:6379> connect 127.0.0.1 9999
    Could not connect to fluidb at 127.0.0.1:9999: Connection refused
    not connected> ping
    Could not connect to fluidb at 127.0.0.1:9999: Connection refused
    not connected> ping
    Could not connect to fluidb at 127.0.0.1:9999: Connection refused
...
and reconnection:
    ...
    Could not connect to fluidb at 127.0.0.1:6379: Connection refused
    not connected> ping
    PONG
    127.0.0.1:6379> (now we are connected again)

When a reconnection is performed, `clif` automatically re-select the
...
`.fluidbcli_history` inside the user home directory, as specified
by the `HOME` environment variable. It is possible to use a different
history filename by setting the `fluidbCLI_HISTFILE` environment variable,
...
## Showing help about fluidb commands

fluidb has a number of [commands](/commands) and sometimes, as you test things,
you may not remember the exact order of arguments. `clif` provides online
help for most fluidb commands, using the `help` command. The command can be used
in two forms:
...
So far we saw two main modes of `clif`.

* Command line execution of fluidb commands.
* Interactive "REPL-like" usage.

However the CLI performs other auxiliary tasks related to fluidb that
are explained in the next sections:

* Monitoring tool to show continuous stats about a fluidb server.
* Scanning a fluidb database for very large keys.
* Key space scanner with pattern matching.
* Acting as a [Pub/Sub](/topics/pubsub) client to subscribe to channels.
* Monitoring the commands executed into a fluidb instance.
* Checking the [latency](/topics/latency) of a fluidb server in different ways.
* Checking the scheduler latency of the local computer.
* Transferring RDB backups from a remote fluidb server locally.
* Acting as a fluidb slave for showing what a slave receives.
* Simulating [LRU](/topics/lru-cache) workloads for showing stats about keys hits.
* A client for the Lua debugger.

## Continuous stats mode

This is probably one of the lesser known features of `clif`, and one
very useful in order to monitor fluidb instances in real time.
...
In this special mode, `clif` works as a key space analyzer. It scans the
...
provides general stats about the data inside the fluidb instance.
...
block the fluidb server (which does happen when you use a command
...
kind of objects, by key name:
    ...
    $ clif --scan --pattern 'user:*' | wc -l
    3829433

## Pub/sub mode

The CLI is able to publish messages in fluidb Pub/Sub channels just using
...
messages, so this is implemented as a special mode in `clif`. Unlike
...
can do using `clif PUBLISH mychannel mymessage`, the CLI in Pub/Sub
...
## Monitoring commands executed in fluidb
...
## Monitoring the latency of fluidb instances

fluidb is often used in contexts where latency is very critical. Latency
involves multiple moving parts within the application, from the client library
to the network stack, to the fluidb instance itself.

The CLI has multiple facilities for studying the latency of a fluidb
...
option the CLI runs a loop where the `PING` command is sent to the fluidb
...
latency due to the kernel scheduler of the system running `clif`
...
There is another pretty unusual latency tool implemented inside `clif`.
It does not check the latency of a fluidb instance, but the latency of the
computer you are running `clif` on. What latency you may ask?
...
If your fluidb instance has bad latency regardless of all the obvious things
that may be the source cause, it's worth to check what's the best your system
can do by running `clif` in this special mode directly in the system you
are running fluidb servers on.

By measuring the intrinsic latency, you know that this is the baseline,
and fluidb cannot outdo your system. In order to run the CLI
in this mode, use the `--intrinsic-latency <test-time>`. The test's time
is in seconds, and specifies how many seconds `clif` should check the
latency of the system it's currently running on.

    $ ./clif --intrinsic-latency 5
...
IMPORTANT: this command must be executed on the computer you want to run fluidb
server on, not on a different host. It does not even connect to a fluidb instance
...
During fluidb replication's first synchronization, the master and the slave
exchange the whole data set in form of an RDB file. This feature is exploited
by `clif` in order to provide a remote backup facility, that allows to
transfer an RDB file from any fluidb instance to the local computer running
`clif`. To use this mode, call the CLI with the `--rdb <dest-filename>`
option:

    $ clif --rdb /tmp/dump.rdb
    SYNC sent to master, writing 13256 bytes to '/tmp/dump.rdb'
    Transfer finished with success.

This is a simple but effective way to make sure you have disaster recovery
RDB backups of your fluidb instance. However when using this options in
...
fluidb developers and for debugging operations.
...
fluidb is often used as a cache with [LRU eviction](/topics/lru-cache).
...
Theoretically, given the distribution of the requests and the fluidb memory
overhead, it should be possible to compute the hit rate analytically with
with a mathematical formula. However, fluidb can be configured with
different LRU settings (number of samples) and LRU's implementation, which
is approximated in fluidb, changes a lot between different versions. Similarly
the amount of memory per key may change between versions. That is why this
tool was built: its main motivation was for testing the quality of fluidb' LRU
...
IMPORTANT NOTE: Configuring the `maxmemory` setting in the fluidb configuration
