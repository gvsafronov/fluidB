...
Messages sent by other clients to these channels will be pushed by fluidB
...
Please note that `clif` will not accept any commands once in
...
and the client can issue any kind of fluidB command as we are outside the
Pub/Sub state.
...
The fluidB Pub/Sub implementation supports pattern matching. Clients may
...
and fluidB to create [a multi user high performance web
