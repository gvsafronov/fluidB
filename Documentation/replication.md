...
At the base of fluidB replication there is a very simple to use and configure *leader follower* (master-slave) replication: it allows slave fluidB instances to be exact copies of master instances. The slave will automatically reconnect to the master every time the link breaks, and will attempt to be an exact copy of it *regardless* of what happens to the master.
...
fluidB uses by default asynchronous replication, which being low latency and
high performance, is the natural replication mode for the vast majority of fluidB
use cases. However fluidB slaves asynchronously acknowledge the amount of data
...
specified number of acknowledged copies in the other fluidB instances, it does not
turn a set of fluidB instances into a CP system with strong consistency: acknowledged
writes can still be lost during a failover, depending on the exact configuration
of the fluidB persistence. However with `WAIT` the probability of losign a write
after a failure event is greatly reduced to certain hard to trigger failure
modes.

You could check the Sentinel or fluidB Cluster documentation for more information
about high availability and failover. However starting with Active-Replication is encouraged as it is simple to use and a powerful tool for high availability. The rest of this document mainly describe the basic characteristics of fluidB basic replication.

The following are some very important facts about fluidB replication:

* fluidB uses asynchronous replication, with asynchronous slave-to-master acknowledges of the amount of data processed.
* A master can have multiple slaves.
* Slaves are able to accept connections from other slaves. Aside from connecting a number of slaves to the same master, slaves can also be connected to other slaves in a cascading-like structure. Since fluidB 4.0, all the sub-slaves will receive exactly the same replication stream from the master.
* fluidB replication is non-blocking on the master side. This means that the master will continue to handle queries when one or more slaves perform the initial synchronization or a partial resynchronization.
* Replication is also largely non-blocking on the slave side. While the slave is performing the initial synchronization, it can handle queries using the old version of the dataset, assuming you configured fluidB to do so in redis.conf.  Otherwise, you can configure fluidB slaves to return an error to clients if the replication stream is down. However, after the initial sync, the old dataset must be deleted and the new one must be loaded. The slave will block incoming connections during this brief window (that can be as long as many seconds for very large datasets). Since fluidB 4.0 it is possible to configure fluidB so that the deletion of the old data set happens in a different thread, however loading the new initial dataset will still happen in the main thread and block the slave.
...
In setups where fluidB replication is used, it is strongly advised to have
...
When fluidB Sentinel is used for high availability, also turning off persistence
...
How fluidB replication works
---

Every fluidB master has a replication ID: it is a large pseudo random string
...
The master starts a background saving process in order to produce an RDB file. At the same time it starts to buffer all new write commands received from the clients. When the background saving is complete, the master transfers the database file to the slave, which saves it on disk, and then loads it into memory. The master will then send all buffered commands to the slave. This is done as a stream of commands and is in the same format of the fluidB protocol itself.

You can try it yourself via telnet. Connect to the fluidB port while the
...
newer fluidB instances, but is still there for backward compatibility: it does
...
The reason why fluidB instances have two replication IDs is because of slaves
...
fluidB version 2.8.18 is the first version to have support for diskless
...
To configure basic fluidB replication is trivial: just add the following line to the slave configuration file:
...
`redis.conf` shipped with the fluidB distribution for more information.
...
parameter. Please refer to the example `redis.conf` file in the fluidB distribution
for more details.

Read-only slave
---

Since fluidB 2.6, slaves support a read-only mode that is enabled by default.
...
fluidB 4.0 RC3 and greater versions totally solve this problem and now writable
slaves are able to evict keys with TTL as masters do, with the exceptions
of keys written in DB numbers greater than 63 (but by default fluidB instances
only have 16 databases).

Also note that since fluidB 4.0 slave writes are only local, and are not propagated to sub-slaves attached to the instance. Sub slaves instead will always receive the replication stream identical to the one sent by the top-level master to the intermediate slaves. So for example in the following setup:
...
To do it on a running instance, use `fluidB-cli` and type:
...
Starting with fluidB 2.8, it is possible to configure a fluidB master to
accept write queries only if at least N slaves are currently connected to the
master.

However, because fluidB uses asynchronous replication it is not possible to ensure
the slave actually received a given write, so there is always a window for data
loss.

This is how the feature works:

* fluidB slaves ping the master every second, acknowledging the amount of replication stream processed.
* fluidB masters will remember the last time it received a ping from every slave.
...
fluidB source distribution.

How fluidB replication deals with expires on keys
---

fluidB expires allow keys to have a limited time to live. Such a feature depends
on the ability of an instance to count the time, however fluidB slaves correctly
replicate keys with expires, even when such keys are altered using Lua
scripts.

To implement such a feature fluidB cannot rely on the ability of the master and
slave to have synchronized clocks, since this is a problem that cannot be solved
and would result into race conditions and diverging data sets, so fluidB
...
When Docker, or other types of containers using port forwarding, or Network Address Translation is used, fluidB replication needs some extra care, especially when using fluidB Sentinel or other systems where the master `INFO` or `ROLE` commands output are scanned in order to discover slaves addresses.
...
In order to fix both issues, it is possible, since fluidB 3.2.2, to force
...
And are documented in the example `redis.conf` of recent fluidB distributions.

The INFO and ROLE command
---

There are two fluidB commands that provide a lot of information on the current
...
Since fluidB 4.0, when an instance is promoted to master after a failover,
