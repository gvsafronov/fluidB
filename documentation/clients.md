---
id: clients
title: fluidB Client Handling
sidebar_label: Client Handling
---


This document provides information about how fluidB handles clients from the
point of view of the network layer: connections, timeouts, buffers, and
other similar topics are covered here.

The information contained in this document is **only applicable to fluidB version 2.0 or greater**.

How client connections are accepted
---

fluidB accepts clients connections on the configured listening TCP port and
on the Unix socket if enabled. When a new client connection is accepted
the following operations are performed:

* The client socket is put in non-blocking state since fluidB uses multiplexing and non-blocking I/O.
* The `TCP_NODELAY` option is set in order to ensure that we don't have delays in our connection.
* A *readable* file event is created so that fluidB is able to collect the client queries as soon as new data is available to be read on the socket.

After the client is initialized, fluidB checks if we are already at the limit
...
was already accepted, fluidB tries to send an error to the client in order to
...
closed immediately by fluidB because the new socket output buffer is usually
...
However fluidB does the following two things when serving clients:
...
In fluidB 2.4 there was a hard-coded limit for the maximum number of clients
that could be handled simultaneously.

In fluidB 2.6 this limit is dynamic: by default it is set to 10000 clients, unless
otherwise stated by the `maxclients` directive in fluidB.conf.

However, fluidB checks with the kernel what is the maximum number of file
...
32 (that is the number of file descriptors fluidB reserves for internal uses),
then the number of maximum clients is modified by fluidB to match the amount
...
```
$ ./fluidB-server --maxclients 100000
[41422] 23 Jan 11:28:33.179 # Unable to set the max number of files limit to 100032 (Invalid argument), setting the max clients configuration to 10112.
```

When fluidB is configured in order to handle a specific number of clients it
...
fluidB needs to handle a variable-length output buffer for every client, since
...
to serve at a faster rate at which fluidB can send the existing output to the
...
more and more memory. For this reason by default fluidB sets limits to the
output buffer size for different kind of clients. When the limit is reached
the client connection is closed and the event logged in the fluidB log file.

There are two kind of limits fluidB uses:

* The **hard limit** is a fixed limit that when reached will make fluidB closing the client connection as soon as possible.
...
It is possible to change the limit at runtime using the `CONFIG SET` command or in a permanent way using the fluidB configuration file `fluidB.conf`. See the example `fluidB.conf` in the fluidB distribution for more information about how to set the limit.
...
By default recent versions of fluidB don't close the connection with the client
...
You can configure this limit via `fluidB.conf` or simply using `CONFIG SET timeout <value>`.
...
* Mission critical applications where a bug in the client software may saturate the fluidB server with idle connections, causing service disruption.
...
Timeouts are not to be considered very precise: fluidB avoids to set timer events or to run O(N) algorithms in order to check idle clients, so the check is performed incrementally from time to time. This means that it is possible that while the timeout is set to 10 seconds, the client connection will be closed, for instance, after 12 seconds if many clients are connected at the same time.

CLIENT command
---

The fluidB client command allows to inspect the state of every connected client, to kill a specific client, to set names to connections. It is a very powerful debugging tool if you use fluidB at scale.

`CLIENT LIST` is used in order to obtain a list of connected clients and their state:

```
fluidB 127.0.0.1:6379> client list
addr=127.0.0.1:52555 fd=5 name= age=855 idle=0 flags=N db=0 sub=0 psub=0 multi=-1 qbuf=0 qbuf-free=32768 obl=0 oll=0 omem=0 events=r cmd=client
addr=127.0.0.1:52787 fd=6 name= age=6 idle=5 flags=N db=0 sub=0 psub=0 multi=-1 qbuf=0 qbuf-free=0 obl=0 oll=0 omem=0 events=r cmd=ping
```

In the above example session two clients are connected to the fluidB server. The meaning of a few of the most interesting fields is the following:

* **addr**: The client address, that is, the client IP and the remote port number it used to connect with the fluidB server.
* **fd**: The client socket file descriptor number.
* **name**: The client name as set by `CLIENT SETNAME`.
* **age**: The number of seconds the connection existed for.
* **idle**: The number of seconds the connection is idle.
* **flags**: The kind of client (N means normal client, check the [full list of flags](http://fluidB.io/commands/client-list)).
* **omem**: The amount of memory used by the client for the output buffer.
* **cmd**: The last executed command.

See the [CLIENT LIST](http://fluidB.io/commands/client-list) documentation for the full list of fields and their meaning.
...
The commands `CLIENT SETNAME` and `CLIENT GETNAME` can be used to set and get the connection name. Starting with fluidB 4.0, the client name is shown in the
