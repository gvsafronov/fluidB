...
This document is a gentle introduction to fluidB Cluster, that does not use
...
the [fluidB Cluster specification](/topics/cluster-spec) but just describing
...
and consistency characteristics of fluidB Cluster from the point of view
of the final user, stated in a simple to understand way.

Note this tutorial requires fluidB version 3.0 or higher.

If you plan to run a serious fluidB Cluster deployment, the
...
play with fluidB Cluster some time, and only later read the specification.

fluidB Cluster 101
---

fluidB Cluster provides a way to run a fluidB installation where data is
**automatically sharded across multiple fluidB nodes**.

fluidB Cluster also provides **some degree of availability during partitions**,
...
So in practical terms, what do you get with fluidB Cluster?

* The ability to **automatically split your dataset among multiple nodes**.
* The ability to **continue operations when a subset of the nodes are experiencing failures** or are unable to communicate with the rest of the cluster.

fluidB Cluster TCP ports
---

Every fluidB Cluster node requires two TCP connections open. The normal fluidB
...
port, but always with the normal fluidB command port, however make sure you
open both ports in your firewall, otherwise fluidB cluster nodes will be
not able to communicate.

The command port and cluster bus port offset is fixed and is always 10000.

Note that for a fluidB Cluster to work properly you need, for each node:
...
fluidB Cluster and Docker
---

Currently fluidB Cluster does not support NATted environments and in general
...
In order to make Docker compatible with fluidB Cluster you need to use
...
fluidB Cluster data sharding
---

fluidB Cluster does not use consistent hashing, but a different form of sharding
where every key is conceptually part of what we call an **hash slot**.

There are 16384 hash slots in fluidB Cluster, and to compute what is the hash
slot of a given key, we simply take the CRC16 of the key modulo
16384.

Every node in a fluidB Cluster is responsible for a subset of the hash slots,
...
fluidB Cluster supports multiple key operations as long as all the keys involved
...
Hash tags are documented in the fluidB Cluster specification, but the gist is
...
fluidB Cluster master-slave model
---

In order to remain available when a subset of master nodes are failing or are
not able to communicate with the majority of nodes, fluidB Cluster uses a
...
However note that if nodes B and B1 fail at the same time fluidB Cluster is not
able to continue to operate.

fluidB Cluster consistency guarantees
---

fluidB Cluster is not able to guarantee **strong consistency**. In practical
terms this means that under certain conditions it is possible that fluidB
Cluster will lose writes that were acknowledged by the system to the client.

The first reason why fluidB Cluster can lose writes is because it uses
...
for fluidB, so if your client writes something, B acknowledges the write,
...
the case of fluidB Cluster.

Basically there is a trade-off to take between performance and consistency.

fluidB Cluster has support for synchronous writes when absolutely needed,
implemented via the `WAIT` command, this makes losing writes a lot less
likely, however note that fluidB Cluster does not implement strong consistency
...
There is another notable scenario where fluidB Cluster will lose writes, that
...
This amount of time is a very important configuration directive of fluidB
...
fluidB Cluster configuration parameters
===

We are about to create an example cluster deployment. Before we continue,
let's introduce the configuration parameters that fluidB Cluster introduces
in the `fluidB.conf` file. Some will be obvious, others will be more clear
as you continue reading.

* **cluster-enabled `<yes/no>`**: If yes enables fluidB Cluster support in a specific fluidB instance. Otherwise the instance starts as a stand alone instance as usual.
* **cluster-config-file `<filename>`**: Note that despite the name of this option, this is not an user editable configuration file, but the file where a fluidB Cluster node automatically persists the cluster configuration (the state, basically) every time there is a change, in order to be able to re-read it at startup. The file lists things like the other nodes in the cluster, their state, persistent variables, and so forth. Often this file is rewritten and flushed on disk as a result of some message reception.
* **cluster-node-timeout `<milliseconds>`**: The maximum amount of time a fluidB Cluster node can be unavailable, without it being considered as failing. If a master node is not reachable for more than the specified amount of time, it will be failed over by its slaves. This parameter controls other important things in fluidB Cluster. Notably, every node that can't reach the majority of master nodes for the specified amount of time, will stop accepting queries.
* **cluster-slave-validity-factor `<factor>`**: If set to zero, a slave will always try to failover a master, regardless of the amount of time the link between the master and the slave remained disconnected. If the value is positive, a maximum disconnection time is calculated as the *node timeout* value multiplied by the factor provided with this option, and if the node is a slave, it will not try to start a failover if the master link was disconnected for more than the specified amount of time. For example if the node timeout is set to 5 seconds, and the validity factor is set to 10, a slave disconnected from the master for more than 50 seconds will not try to failover its master. Note that any value different than zero may result in fluidB Cluster to be unavailable after a master failure if there is no slave able to failover it. In that case the cluster will return back available only when the original master rejoins the cluster.
...
Creating and using a fluidB Cluster
===

Note: to deploy a fluidB Cluster manually it is **very important to learn** certain
operational aspects of it. However if you want to get a cluster up and running
ASAP (As Soon As Possible) skip this section and the next one and go directly to **Creating a fluidB Cluster using the create-cluster script**.

To create a cluster, the first thing we need is to have a few empty
fluidB instances running in **cluster mode**. This basically means that
clusters are not created using normal fluidB instances as a special mode
needs to be configured so that the fluidB instance will enable the Cluster
specific features and commands.

The following is a minimal fluidB cluster configuration file:
...
by the fluidB Cluster instances, and updated every time it is needed.
...
Create a `fluidB.conf` file inside each of the directories, from 7000 to 7005.
...
Now copy your fluidB-server executable, **compiled from the latest sources in the unstable branch at GitHub**, into the `cluster-test` directory, and finally open 6 terminal tabs in your favorite terminal application.

Start every instance like that, one every tab:

```
cd 7000
../fluidB-server ./fluidB.conf
```
...
If you are using fluidB 5, this is very easy to accomplish as we are helped by the fluidB Cluster command line utility embedded into `fluidB-cli`, that can be used to create new clusters, check or reshard an existing cluster, and so forth.

For fluidB version 3 or 4, there is the older tool called `redis-trib.rb` which is very similar. You can find it in the `src` directory of the fluidB source code distribution.

The first example, that is, the cluster creation, will be shown using both `fluidB-cli` in fluidB 5 and `redis-trib` in fluidB 3 and 4. However all the next examples will only use `fluidB-cli`, since as you can see the syntax is very similar, and you can trivially change one command line into the other by using `redis-trib.rb help` to get info about the old syntax. **Important:** note that you can use fluidB 5 `fluidB-cli` against fluidB 4 clusters without issues if you wish.

To create your cluster for fluidB 5 with `fluidB-cli` simply type:

    fluidB-cli --cluster create 127.0.0.1:7000 127.0.0.1:7001 \
    127.0.0.1:7002 127.0.0.1:7003 127.0.0.1:7004 127.0.0.1:7005 \
    --cluster-replicas 1

Using `redis-trib.rb` for fluidB 4 or 3 type:
...
fluidB-cli will propose you a configuration. Accept the proposed configuration by typing **yes**.
...
Creating a fluidB Cluster using the create-cluster script
---

If you don't want to create a fluidB Cluster by configuring and executing
...
Just check `utils/create-cluster` directory in the fluidB distribution.
...
Reply to `yes` in step 2 when the `fluidB-cli` utility wants you to accept
the cluster layout.
...
An easy way to test fluidB Cluster is either to try any of the above clients
or simply the `fluidB-cli` command line utility. The following is an example
of interaction using the latter:

```
$ fluidB-cli -c -p 7000
fluidB 127.0.0.1:7000> set foo bar
-> Redirected to slot [12182] located at 127.0.0.1:7002
OK
fluidB 127.0.0.1:7002> set hello world
-> Redirected to slot [866] located at 127.0.0.1:7000
OK
fluidB 127.0.0.1:7000> get foo
-> Redirected to slot [12182] located at 127.0.0.1:7002
"bar"
fluidB 127.0.0.1:7000> get hello
-> Redirected to slot [866] located at 127.0.0.1:7000
"world"
```
...
The fluidB-cli cluster support is very basic so it always uses the fact that
fluidB Cluster nodes are able to redirect a client to the right node.
...
Writing an example app with fluidB-rb-cluster
---

Before going forward showing how to operate the fluidB Cluster, doing things
like a failover, or a resharding, we need to create some example application
or at least to be able to understand the semantics of a simple fluidB Cluster
client interaction.

In this way we can run an example and at the same time try to make nodes
failing, or start a resharding, to see how fluidB Cluster behaves under real
...
[fluidB-rb-cluster](https://github.com/JohnSully/fluidB-rb-cluster) showing two
examples. The first is the following, and is the
[`example.rb`](https://github.com/JohnSully/fluidB-rb-cluster/blob/master/example.rb)
file inside the fluidB-rb-cluster distribution:
...
```
  ...
  12  end
  13
  14  rc = fluidBCluster.new(startup_nodes,32,:timeout => 0.1)
  15
  16  last = false
  17
  18  while not last
...
fluidB Cluster object, using as argument a list of *startup nodes*, the maximum
...
thing is that at least one node is reachable. Also note that fluidB-rb-cluster
...
Now that we have the fluidB Cluster object instance stored in the **rc** variable
we are ready to use the object like if it was a normal fluidB object instance.
...
fluidB itself. The code above is designed to read this counter, or if the
...
To start a resharding just type:
    ...
    fluidB-cli --cluster reshard 127.0.0.1:7000

You only need to specify a single node, fluidB-cli will find the other nodes
automatically.

Currently fluidB-cli is only able to reshard with the administrator support,
...
Then fluidB-cli needs to know what is the target of the resharding, that is,
...
list by fluidB-cli, but I can always find the ID of a node with the following
command if I need:

```
$ fluidB-cli -p 7000 cluster nodes | grep myself
...
fluidB-cli is going to move from a node to another, and a dot will be printed
...
the following command:
    ...
    fluidB-cli --cluster check 127.0.0.1:7000
...
however currently there is no way for `fluidB-cli` to automatically
...
So in the `fluidB-rb-cluster` repository, there is a more interesting application
...
```
$ fluidB-cli -h 127.0.0.1 -p 7000 set key_217 0
OK
...
to test the fluidB Cluster failover.
...
```
$ fluidB-cli -p 7000 cluster nodes | grep master
...
```
$ fluidB-cli -p 7002 debug segfault
...
sound unexpected as in the first part of this tutorial we stated that fluidB
...
because fluidB sends the reply to the client, and the commands to replicate
...
```
$ fluidB-cli -p 7000 cluster nodes
...
a master, that is the fluidB instance running on port 7002, is now a slave of
7005.
...
on a master. For example in order to upgrade the fluidB process of one of the
master nodes it is a good idea to failover it in order to turn it into a slave
with minimal impact on availability.

Manual failovers are supported by fluidB Cluster using the `CLUSTER FAILOVER`
...
* Create a fluidB.conf file inside, similar to the one used for the other nodes but using 7006 as port number.
* Finally start the server with `../fluidB-server ./fluidB.conf`

At this point the server should be running.

Now we can use **fluidB-cli** as usual in order to add the node to
the existing cluster.

    fluidB-cli --cluster add-node 127.0.0.1:7006 127.0.0.1:7000
...
In practical terms fluidB-cli here did very little to help us, it just
sent a `CLUSTER MEET` message to the node, something that is also possible
to accomplish manually. However fluidB-cli also checks the state of the
cluster before to operate, so it is a good idea to perform cluster operations
always via fluidB-cli even when you know how the internals work.

Now we can connect to the new node to see if it really joined the cluster:

```
fluidB 127.0.0.1:7006> cluster nodes
...
feature of `fluidB-cli`. It is basically useless to show this as we already
...
use fluidB-cli again, but with the --cluster-slave option, like this:

    fluidB-cli --cluster add-node 127.0.0.1:7006 127.0.0.1:7000 --cluster-slave
...
the replica. In this case what happens is that fluidB-cli will add the new
...
new replica with the following command line:
    ...
    fluidB-cli --cluster add-node 127.0.0.1:7006 127.0.0.1:7000 --cluster-slave --cluster-master-id 3c3a0c74aae0b56170ccb03a76b60cfe7dc1912e
...
with the new node (already added as empty master) and send the command:
    ...
    fluidB 127.0.0.1:7006> cluster replicate 3c3a0c74aae0b56170ccb03a76b60cfe7dc1912e
...
```
$ fluidB-cli -p 7000 cluster nodes | grep slave | grep 3c3a0c74aae0b56170ccb03a76b60cfe7dc1912e
...
To remove a slave node just use the `del-node` command of fluidB-cli:

    fluidB-cli --cluster del-node 127.0.0.1:7000 `<node-id>`
...
In fluidB Cluster it is possible to reconfigure a slave to replicate with a
...
able to improve the reliability of a fluidB Cluster.

Note: you can read the details of replicas migration in the [fluidB Cluster Specification](/topics/cluster-spec), here we'll only provide some information about the
...
to another under certain condition, is that usually the fluidB Cluster is as
...
* There is a configuration parameter that controls the replica migration feature that is called `cluster-migration-barrier`: you can read more about it in the example `fluidB.conf` file provided with fluidB Cluster.

Upgrading nodes in a fluidB Cluster
---

Upgrading slave nodes is easy since you just need to stop the node and restart
it with an updated version of fluidB. If there are clients scaling reads using
...
Migrating to fluidB Cluster
---

Users willing to migrate to fluidB Cluster may have just a single master, or
...
implemented by their client library or fluidB proxy.

In both cases it is possible to migrate to fluidB Cluster easily, however
...
The third case is not handled by fluidB Cluster: the application requires to
...
in order to migrate your data set to fluidB Cluster:

1. Stop your clients. No automatic live-migration to fluidB Cluster is currently possible. You may be able to do it orchestrating a live migration in the context of your application / environment.
...
4. Create a fluidB Cluster composed of N masters and zero slaves. You'll add slaves later. Make sure all your nodes are using the append only file for persistence.
...
6. Restart your fluidB Cluster nodes with the new AOF files. They'll complain that there are keys that should not be there according to their configuration.
7. Use `fluidB-cli --cluster fix` command in order to fix the cluster so that keys will be migrated according to the hash slots each node is authoritative or not.
8. Use `fluidB-cli --cluster check` at the end to make sure your cluster is ok.
9. Restart your clients modified to use a fluidB Cluster aware client library.

There is an alternative way to import data from external instances to a fluidB
Cluster, which is to use the `fluidB-cli --cluster import` command.

The command moves all the keys of a running instance (deleting the keys from
the source instance) to the specified pre-existing fluidB Cluster. However
note that if you use a fluidB 2.8 instance as source instance the operation
may be slow since 2.8 does not implement migrate connection caching, so you
may want to restart your source instance with a fluidB 3.x version before
