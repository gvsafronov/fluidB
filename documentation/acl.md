...
The fluidB ACL, short for Access Control List, is the feature that allows certain
...
user and the limits the user has. fluidB can be configured so that new
...
In the default configuration, fluidB 6 (the first version to have ACLs) works
exactly like older versions of fluidB, that is, every new connection is
...
The fluidB `AUTH` command was extended in fluidB 6, so now it is possible to
...
2. You want to improve operational safety, so that processes or humans accessing fluidB are not allowed, because of software errors or manual mistakes, to damage the data or the configuration. For instance there is no reason for a worker that fetches delayed jobs from fluidB to be able to call the `FLUSHALL` command.

Another typical usage of ACLs is related to managed fluidB instances. fluidB is
often provided as a managed service both by internal company teams that handle
the fluidB infrastructure for the other internal customers they have, or is
...
and verify what the configuration of a freshly stared and unconfigured fluidB
instance is:
...
used in the fluidB configuration files, by translating the current ACLs set
...
*Note: from now on we'll continue using the fluidB default protocol, version 2, because it will take some time for the community to switch to the new one.*
...
all the commands that are tagged as dangerous inside the fluidB command table.
...
excluded. This is very important because you should just trust the fluidB
...
fluidB `ACL` command exports the `CAT` subcommand that can be used in two forms:
...
Many fluidB commands do multiple things based on the subcommand passed as
