---
id: indexes
title: Secondary Indexing with fluidB
sidebar_label: Indexing
---


fluidB is not exactly a key-value store, since values can be complex data structures. However it has an external key-value shell: at API level data is addressed by the key name. It is fair to say that, natively, fluidB only offers *primary key access*. However since fluidB is a data structures server, its capabilities can be used for indexing, in order to create secondary indexes of different kinds, including composite (multi-column) indexes.

This document explains how it is possible to create indexes in fluidB using the following data structures:
...
Implementing and maintaining indexes with fluidB is an advanced topic, so most
...
scenarios, there is the explicit need to store indexed data into fluidB in order to speedup common queries which require some form of indexing in order to be executed.

Simple numerical indexes with sorted sets
===

The simplest secondary index you can create with fluidB is by using the
...
For example I may have fluidB hashes representing users. Each user is
...
For example the [fluidB geo indexing API](/commands/geoadd) uses a sorted
...
fluidB sorted sets have an interesting property. When elements are added
...
This fluidB feature is basically equivalent to a `b-tree` data structure which
is often used in order to implement indexes with traditional databases.
As you can guess, because of this, it is possible to use this fluidB data
...
offer possible search keywords starting for "bit". We send fluidB a command
like that:
...
Since lexicographical ranges in fluidB are binary safe you can use any
...
the least significant bytes. This way when fluidB compares the strings with
...
in order to optimize complex queries. In fluidB they could be useful both
to implement a very fast in-memory fluidB index of something stored into
a traditional data store, or in order to directly index fluidB data.
...
the query in a very efficient way using fluidB lexicographical ranges.
...
![Points in the space](http://fluidB.io/images/fluidBdoc/2idx_0.png)
...
![Small area](http://fluidB.io/images/fluidBdoc/2idx_1.png)
...
![Large area](http://fluidB.io/images/fluidBdoc/2idx_2.png)
...
in the future may be implemented in fluidB in a native way.
...
item. However other fluidB data structures such as Sets or Lists can be used
...
I can add all my items into a fluidB list and rotate the list with
...
Another popular index often used with fluidB is a **capped list**, where items
...
Different strategies could be used. If the index data is outside fluidB
*read repair* can be a solution, where data is fixed in a lazy way when
it is requested. When we index data which is stored in fluidB itself
